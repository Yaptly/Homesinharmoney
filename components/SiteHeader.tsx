  ...
  return (
    <header className="w-full">
      <div className="max-w-6xl mx-auto px-6 py-6 flex items-center justify-between gap-4">
        <Link href="/" className="font-display text-xl text-sage-deep tracking-tight">Homes In Harmony</Link>
        ...
        <Link href="/book" className="tracked-caps bg-sage-deep text-cream px-5 py-3 rounded-full hover:bg-[#324a2c] transition-colors">Book Now</Link>
      </div>
      <nav aria-label="Mobile navigation" className="md:hidden px-6 pb-4 flex flex-wrap gap-x-5 gap-y-2 tracked-caps text-charcoal-soft text-xs">
        <Link href="/services">Services</Link><Link href="/service-areas">Areas</Link><Link href="/about">About</Link><Link href="/contact">Contact</Link>
      </nav>
    </header>
  );
}
